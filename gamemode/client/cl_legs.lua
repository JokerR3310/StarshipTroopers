local legsconvar = CreateConVar( "cl_legsconvar", "0", { FCVAR_ARCHIVE, }, "Enable/Disable the legs" )
 
local Legs = {}
Legs.LegEnt = nil

function ShouldDrawLegs()
	if !legsconvar:GetBool() then return end
...
hook.Add("PlayerSpawn", "fixthisshit", function(ply)
	if ply == LocalPlayer() then
		Legs:SetUp()
		self.LegEnt = nil
	end
